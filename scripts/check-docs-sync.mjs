...
// The second half of the same convention: a visual change has to SHOW itself.
// A computed-style table proves a value moved; it cannot prove the result looks
// right, and that is the thing a reviewer is actually being asked to judge.
//
// Deliberately cheap to satisfy and deliberately not clever: one image, and the
// body has to name both states. A single side-by-side composite is the usual
// shape here, so counting images would be the wrong test — the words are what
// say a comparison was made rather than a screenshot dropped in.
const SHOT_SKIP_MARKER = '[skip screenshots]'
const SHOT_SKIP_RE = /^[ \t>*-]*\[skip screenshots\]/im
const IMG_RE = /!\[[^\]]*\]\([^)]+\)|<img[\s>]/i
const BEFORE_RE = /\bbefore\b|改动前|修改前|之前/i
const AFTER_RE = /\bafter\b|改动后|修改后|之后/i
...
function main() {
  const body = process.env.DOCS_SYNC_PR_BODY || ''
  const skipReason = SKIP_RE.test(body)
  const shotSkip = SHOT_SKIP_RE.test(body)
  // No body means this is a local run, not a PR. The docs half still works
  // from the diff alone; the screenshot half has nothing to read, and failing
  // it locally would only teach people to stop running the script.
  const haveBody = body.trim().length > 0
  ...
  // Screenshots: only meaningful once something visual actually changed.
  let shotMissing = null
  if (touched.size && haveBody && !shotSkip) {
    const hasImg = IMG_RE.test(body)
    const hasBoth = BEFORE_RE.test(body) && AFTER_RE.test(body)
    if (!hasImg || !hasBoth) shotMissing = { hasImg, hasBoth }
  }

  if (JSON_OUT) {
    console.log(
      JSON.stringify(
        { base: DIFF_BASE, skipped: skipReason, violations, undocumented, screenshots: shotMissing ?? 'ok' },
        null,
        2,
      ),
    )
    const bad = (violations.length && !skipReason) || shotMissing
    return bad ? 1 : 0
  }
  ...
  const reportShots = () => {
    if (!shotMissing) return
    console.log('\nThis PR changes how something looks, but its description does not show it.\n')
    if (!shotMissing.hasImg) console.log('  missing: a before/after image in the PR body')
    else console.log('  found an image, but the body never names both states')
    console.log(
      '\nAdd the two renderings — the same specimen off `main` and off this branch,\n' +
        'same viewport and same clip — and label which is which. A table of computed\n' +
        'styles proves a value moved; it cannot show that the result looks right.\n' +
        `\nIf this change genuinely has nothing to show, start a line of the PR body\n` +
        `with "${SHOT_SKIP_MARKER}" followed by the reason.\n`,
    )
  }
  ...
  if (skipReason && !shotMissing) {
    console.log(`Overridden by "${SKIP_MARKER}" — passing anyway.`)
    return 0
  }
  if (skipReason) console.log(`Documentation requirement overridden by "${SKIP_MARKER}".`)
  return 1
}
