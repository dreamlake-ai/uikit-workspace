...
(**Data**). The card *is* the JSON: the kind glyph ← `kind`, the title ← `title`,
...
| `showControls` | `boolean` | `true` | Show the overlay chrome — the legend (top-right: node-kind glyphs + edge-flow swatches) and the keyboard-hint strip (bottom). Pass `false` for tiny embeds. |
