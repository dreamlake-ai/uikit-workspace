...
filename worth labelling and no language to highlight, so this one passes
neither.

With no `filename` there is no header bar — hover the block and Copy fades in
over the top-right corner instead, which is what you want for a traceback
someone is about to paste into an issue. `wrap` soft-wraps rather than
scrolling sideways, and `maxHeight` caps the block and scrolls inside it.

Omitting `lang` is a legitimate choice here rather than a missing prop: plain
text is what this content is. To drop the tools as well, pass `header={false}`.
