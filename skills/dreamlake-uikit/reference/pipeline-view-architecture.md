...
4. **Place param tags.** One tag per node-pair lists the params it transfers,
   resting **on** the edge at its routed bend (`buildEdgePath` reports the
   point, so the tag tracks the line through detours). Dragging a tag along the
   edge rebends it (a per-pair `bendFrac`); across it lifts the tag onto a leader.
5. **Draw nodes.** Absolutely-positioned cards, tinted by `status`, with the
   kind's lucide glyph (`kindIcon`, tinted by `status` — shape means type, colour
   means state), the `inputs→outputs` meta, a status footer (pulsing when
   `running`), and a single input dot (left-centre) / output dot (right-centre) —
   every parameter shares the one input dot; its names live in the param tag.
6. **Overlay chrome** (`showControls`, default on). The legend (top-right) and
   the keyboard-hint strip (bottom). The legend has two sections: **nodes** — the
   kind glyphs, drawn neutral and listing only the kinds present on *this*
   canvas (`NodeKind` is open-ended, so a fixed list would either miss a graph's
   kinds or pad the key with rows you can't see) — and **edges**, rendered from
...
- **Param tags, not per-edge flow-label chips.** The prototype floats a draggable
  ...
  node-pair transfers (dragging it rebends the edge), and the flow vocabulary
  lives in the legend.
- **No auto-fit, no vertical layout mode.** Positions come from the tracer's
  layout; the user pans/zooms/drags nodes.
...
- ✅ **Per-pair param tags** — one tag per node-pair naming the params it
  transfers, resting on its edge; drag it to rebend the edge or lift it onto a
  leader.
...
- **Tag collision avoidance** — tags rest on the edge today; auto-nudging
  crowded ones clear of each other (prototype parity).
