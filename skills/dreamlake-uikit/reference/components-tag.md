...
- **`variant="tag"` (default)** — hashtag presentation. `#name` with an
  ...
  project footers.
- **`variant="pill"`** — the bare label, no `#`; colour carries the meaning
...
A labelled value: no line, no fill, just the word in its tone with room around
it. This is what the app reaches for when a value needs to read as a **labelled object** rather than
as prose: a kind, a run's status, someone's role, a bindr you can take off.

Every one of them is `compact`, and the status row is `light` as well —
`compact light` is the shape in production, not the default size below it.
...
| `active` | `boolean` | `false` | The accent colour takes over: the pill's hairline and label, or the tag's label. Neither fills. **The app does not use this** — every Tag it renders is a passive label — so treat it as available rather than as the shape to design against. |
