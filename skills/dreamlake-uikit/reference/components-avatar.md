...
Pass `radius` (px) to override the simple form's default `4` (rounded square).
...
| `radius` | `number` | `4` / circle | Border radius in px. The simple form defaults to `4` (rounded square); the composed form defaults to a circle. Use `size / 2` for a circle in the simple form. |
