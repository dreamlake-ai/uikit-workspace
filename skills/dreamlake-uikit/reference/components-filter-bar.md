...
sort dropdown. When the user focuses search with a non-`"all"` filter active,
...
search is focused or `query` is non-empty and a non-`"all"` filter is selected, the chips are
replaced by a token pill inside the search field. The token has an accessible × button and no dropdown. Backspace at the start
of the input removes the token without changing the query. Filter chips can carry a state color via
