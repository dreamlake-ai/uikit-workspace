...
export function ContextMenu({ onOpenChange, children }: ContextMenuProps) {
  ...
  const value = useMemo<MenuCtx>(
    () => ({ open, setOpen, refs, floatingStyles, context, getFloatingProps, getItemProps, activeIndex, elementsRef }),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [open, refs, floatingStyles, context, getFloatingProps, getItemProps, activeIndex],
  )
...
export function ContextMenuTrigger({ asChild = false, children, onContextMenu, ...props }: ContextMenuTriggerProps) {
...
export interface ContextMenuContentProps extends ComponentProps<'div'>, LegacyOverlayContentProps {}
...
const itemClass =
  'flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-uikit-12 leading-uikit-snug cursor-pointer select-none outline-none data-[active=true]:bg-uikit-ink-5 data-[danger=true]:text-uikit-tone-red disabled:opacity-50 disabled:pointer-events-none'
...
export function ContextMenuItem({ className, onClick, onSelect, disabled, danger, children, ...props }: ContextMenuItemProps) {
...
export function ContextMenuGroup({ className, ...props }: ComponentProps<'div'>) {
...
export function ContextMenuShortcut({ className, ...props }: ComponentProps<'span'>) {
  return <span className={cn('ml-auto text-uikit-10 text-uikit-muted tracking-uikit-wide', className)} {...props} />
}
