...
const toggleClass =
  'inline-flex items-center justify-center size-8 rounded-md text-uikit-muted hover:bg-uikit-ink-5 hover:text-uikit-ink transition-colors cursor-pointer outline-none'
 ...
 */
export function ThemeColorToggle({ className, ...props }: ThemeColorToggleProps) {
  const { baseTheme, setBaseTheme } = useTheme()
  const next = () => setBaseTheme(baseTheme === 'light' ? 'dark' : baseTheme === 'dark' ? 'system' : 'light')
  return (
    <button
      type="button"
      onClick={next}
      ...
      {...props}
    >
      {baseTheme === 'system' ? <SystemIcon /> : baseTheme === 'dark' ? <MoonIcon /> : <SunIcon />}
    </button>
  )
}
