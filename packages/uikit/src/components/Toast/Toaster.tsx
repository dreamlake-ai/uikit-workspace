  ...
  return (
    <div
      ...
      className={cn(
        ...
        'rounded-[10px] border border-uikit-faint bg-uikit-panel text-uikit-ink',
    ...
    >
      <span className="mt-0.5 flex size-3.5 shrink-0 items-center justify-center">
        {item.type === 'loading' ? (
          ...
          <span className="size-2 rounded-full" style={{ background: TONE[item.type] }} />
        )}
      </span>
      <div className="flex-1 min-w-0">
        {item.title != null && (
          <div className="text-uikit-12 leading-uikit-snug font-medium">{item.title}</div>
        )}
 ...
 */
export function Toaster({ position = 'bottom-right', duration = 4000, className }: ToasterProps) {
  ...
  return createPortal(
    <div
      className={cn('fixed z-[200] flex flex-col gap-2 pointer-events-none', className)}
      style={{ ...POSITION[position], flexDirection: fromTop ? 'column' : 'column-reverse' }}
    >
      {items.map((item) => (
