...
export const InputRoot = forwardRef<HTMLInputElement, InputRootProps>(function InputRoot(
  { style, className, side, children, disabled, state = 'default', size = 'md', type, inputClassName, ...props },
  ref,
) {
  ...
  return (
    <div
      style={style}
      data-input
      className={cn('flex items-center gap-2 overflow-hidden rounded-[10px] text-uikit-ink', surface, SIZE[size], className)}
      onClick={(event) => {
...
export const InputSlot = forwardRef<HTMLDivElement, InputSlotProps>(function InputSlot(
  ...
  return (
    <div
      ...
      {...props}
      ref={ref}
      className={cn('input-slot shrink-0 cursor-text text-uikit-muted', side === 'left' ? '-order-1' : 'order-0', className)}
    >
      {children}
    </div>
  )
