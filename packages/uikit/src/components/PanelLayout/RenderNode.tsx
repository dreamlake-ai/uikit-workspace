...
export function RenderNode({
    ...
    return (
      ...
      >
        {(group.children.length > 1 || showSingleTab?.(group.children[0])) && (
          <div
            data-panel-tab-slot={group.activeId}
            style={{
              position: "absolute",
              left: 10,
              right: 56,
        ...
        <div
          data-panel-slot={group.activeId}
  ...
  if (node.kind === "group") {
    ...
    const active =
      node.children.find((c) => c.id === node.activeId) ?? node.children[0];
    return (
  ...
  return (
    ...
    <div
      ...
      style={{ flexDirection: node.dir }}
    >
      {node.children.map((child, i) => (
        <Fragment key={child.id}>
          {/* flexGrow is the runtime split size — kept inline. */}
          <div
            className="basis-0 shrink min-w-0 min-h-0 flex"
            style={{ flexGrow: node.sizes[i] }}
          >
            <RenderNode
              node={child}
