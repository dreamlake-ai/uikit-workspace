...
export function panelLeaf(
  opts: { view?: string; title?: string; contentMax?: number } = {},
): LeafNode {
...
export function unwrapLayout(raw: unknown): unknown {
  if (
    raw &&
    typeof raw === "object" &&
    "root" in (raw as Record<string, unknown>)
  ) {
...
export function applySplit(
  ...
  before = false,
  newFraction = 0.5,
): { node: PanelNode; newId: string | null } {
  ...
  const f = Number.isFinite(newFraction)
    ? Math.max(0.1, Math.min(0.9, newFraction))
    : 0.5;
  if (node.kind === "leaf") {
  ...
  if (node.kind === "group") {
    if (!node.children.some((c) => c.id === targetId))
      return { node, newId: null };
    const target = node.children.find((c) => c.id === targetId)!;
    const remaining = normalizeGroup({
      ...node,
    ...
    return {
      node: {
        ...
        id: nextId(),
        dir: "row",
        children: [remaining, splitTarget],
        sizes: [50, 50],
      },
      newId: fresh.id,
    };
...
export function applyResize(
  ...
  return {
    ...node,
    children: node.children.map((c) => applyResize(c, splitId, index, a, b)),
  };
}
...
export function extractLeaf(
  node: PanelNode,
  id: string,
): { tree: PanelNode | null; taken: LeafNode | null } {
  if (node.kind === "leaf") {
    return node.id === id
      ? { tree: null, taken: node }
      : { tree: node, taken: null };
  }
  if (node.kind === "group") {
...
export function insertBeside(
  ...
  return {
    ...
    children: node.children.map((c) =>
      insertBeside(c, targetId, incoming, side),
    ),
  };
}
...
export function applyActivateTab(
  ...
  return {
    ...node,
    children: node.children.map((c) => applyActivateTab(c, groupId, leafId)),
  };
}
...
export function dockSideFromPoint(
  rect: DOMRect | PanelRect,
  x: number,
  y: number,
  allowCenter = true,
): DockSide {
  ...
  const CENTER = 0.25;
  if (allowCenter && Math.abs(px - 0.5) < CENTER && Math.abs(py - 0.5) < CENTER)
    return "center";
  const dl = px;
  const dr = 1 - px;
  const dt = py;
