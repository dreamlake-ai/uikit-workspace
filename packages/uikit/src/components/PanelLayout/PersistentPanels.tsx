...
export function PersistentPanels({
    ...
    if (node.kind === "leaf")
      ...
      node.children.forEach((leaf) =>
        leaves.push({
          leaf,
          box,
          inGroup: node.children.length > 1 || !!showSingleTab?.(leaf),
          multiple: node.children.length > 1,
        }),
      );
    else
      node.children.forEach((child, i) =>
        walk(child, splitChildBox(box, node.sizes, i, node.dir)),
      );
  };
  walk(root, rootBox);
  ...
  for (const { leaf } of leaves)
    if (!mountOrder.current.has(leaf.id))
      mountOrder.current.set(leaf.id, sequence.current++);
  leaves.sort(
    (a, b) =>
      mountOrder.current.get(a.leaf.id)! - mountOrder.current.get(b.leaf.id)!,
  );
  useLayoutEffect(() => {
    ...
    const slots = Array.from(
      area.querySelectorAll<HTMLElement>("[data-panel-slot]"),
    );
    const tabSlots = Array.from(
      area.querySelectorAll<HTMLElement>("[data-panel-tab-slot]"),
    );
    const toolbars = Array.from(
      el.querySelectorAll<HTMLElement>("[data-panel-toolbar]"),
    );
    const measure = () => {
      const origin = area.getBoundingClientRect();
      const rects = new Map(
        slots.map((slot) => [
          slot.dataset.panelSlot,
      ...
      for (const tabSlot of tabSlots) {
        const toolbar = toolbars.find(
          (node) => node.dataset.panelToolbar === tabSlot.dataset.panelTabSlot,
        );
