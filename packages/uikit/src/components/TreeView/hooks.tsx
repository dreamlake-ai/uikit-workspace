...
export function isIndirectlySelected<T extends TreeDataItem>(
  ...
  return getAncestorIds(itemId, data).some((ancestorId) =>
    selectedIds.has(ancestorId),
  );
}
...
export function getAdjacentSelectionState<T extends TreeDataItem>(
  ...
  const hasPrevSelected =
    prevIndex >= 0 &&
    getMultiSelectState(data[prevIndex].id, selectedIds, data) !== "unselected";
        ...
        <>
          {parts.map((part, i) =>
            i % 2 === 1 ? (
              <span
                key={i}
                className="rounded-uikit-badge bg-uikit-tone-amber/30"
              >
                {part}
              </span>
            ) : (
