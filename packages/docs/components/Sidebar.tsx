        ...
        <div key={group.label || '_'} className="flex flex-col" style={{ gap: 1, marginBottom: isFolded ? 4 : 14, transition: 'margin-bottom 0.2s ease' }}>
          {group.label && (
            ...
            >
              ...
              <span className={isFolded ? 'opacity-60' : 'opacity-0 group-hover:opacity-60 transition-opacity'}
                style={{ fontSize: 10, fontWeight: 400, lineHeight: 1, flexShrink: 0 }}
              >
                {isFolded ? '+' : '−'}
              </span>
          ...
          {group.items.map(item => (
            <a
              key={item.path}
              href={item.path}
              aria-current={item.path === urlPathname ? 'page' : undefined}
