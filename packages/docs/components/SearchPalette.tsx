...
export function SearchPalette({ open, onClose, query }: SearchPaletteProps) {
  ...
  const { panelH, effectiveH, isResizing, startResize, resetHeight } = useSearchResize(hits.length, totalChars)
  ...
  return (
    <>
      ...
      <div data-palette-panel role="dialog" aria-modal="true" aria-label="Search" style={panelStyle}>
        ...
        {!isNarrowInset && <SearchResizeHandles startResize={startResize} resetHeight={resetHeight} />}
      </div>
    </>
  )
}
