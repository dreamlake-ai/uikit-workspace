...
export function TOC() {
  ...
  // The page's markdown twin (generated by scripts/gen-llms.mjs) — the same
  // content the agent skill reads. Surfaced as a rail link below the TOC.
  // Suppressed on noindex pages, which have no generated .md.
  const mdHref = (() => {
    const meta = pages.find(p => p.path === urlPathname)
    if (!meta || meta.noindex) return null
    return urlPathname === '/' ? '/index.md' : `${urlPathname}.md`
  })()
    ...
    >
      ...
      >
        {mdHref && (
          <a
            href={mdHref}
            // Static .md asset, not a client route — force a full navigation
            // so Vike's SPA router doesn't intercept and 404.
            rel="external"
