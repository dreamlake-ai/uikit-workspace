...
section: Pipeline
order: 248
description: How PipelineGraph is built — the file split, the design principles, the render pipeline, and what it deliberately leaves out — plus the current stage and what's planned next.
---
...
[Pipeline Graph](/pipeline-view/pipeline-graph).
...
- ✅ **Connector tags** (`data` / `mask`) and **connector states** (the six
  derived flows), catalogued in [Anatomy](/pipeline-view/anatomy).
...
[`NodePreview`](/pipeline-view/pipeline-graph-json#the-output-artifact) (schema +
...
**Back to:** [Pipeline Graph](/pipeline-view/pipeline-graph) ·
[Anatomy](/pipeline-view/anatomy) ·
