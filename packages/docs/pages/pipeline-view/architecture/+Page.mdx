...
- **Output** — render the node's [`output` artifact](/pipeline-view/pipeline-graph-json#the-output-artifact):
...
[JSON reference](/pipeline-view/pipeline-graph-json#the-output-artifact) so the
