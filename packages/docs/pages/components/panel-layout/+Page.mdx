...
Use the [layout controller](/components/layout-controller) to address named destinations,
...
<Callout variant="info" title="Give it a height">
  `PanelLayout` fills its container. Without an ancestor that has a real height it collapses to
...
<Callout variant="warn" title="Reset is not the same door as restore">
  `initial` RESTORES a layout, so wiring the toolbar's Reset to it hands back the very layout the
...
<Callout variant="info" title="Docking is pointer-only">
  Dragging a panel onto another's edge — or a tab out of its group — has no keyboard equivalent. The
